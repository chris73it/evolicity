...
// Forward declaration of generic Box class to
...
public:
	static const size_t fromAddress = 0x00000; //0
	static const size_t toAddress   = 0x10000; //65536
	static const size_t amount = toAddress - fromAddress; //65536

	Mos6502();
	~Mos6502() = default;

	// Link this CPU to its bus
	//void ConnectBus(Bus* n) { box = n; }

	// ICpu
	// External event functions. In hardware these represent pins that are asserted
	// to produce a change in state.
	void reset() override;	// Reset Interrupt - Forces CPU into known state
	void irq() override;		// Interrupt Request - Executes an instruction at a specific location
	void nmi() override;		// Non-Maskable Interrupt Request - As above, but cannot be disabled
	void clock() override;	// Perform one clock cycle's worth of update
