...
//const std::array<uint8_t, Mos6502::amount>& Mos6502::getBytes() const
//{
//	//std::cout << "Mos6502::getBytes     const" << std::endl;
//	return bytes;
//}
...
void Mos6502::dump() const
{
	std::cout << "Mos6502::dump" << std::endl;
	uint8_t value;
...
// Reads an 8-bit byte from the box, located at the specified 16-bit address
uint8_t Mos6502::read(size_t address) const
{
...
void Mos6502::write(size_t address, uint8_t byte)
{
...
void Mos6502::clock()
{
	...
	if (cycles == 0)
	{
		...
		opcode = read(pc);
		//std::cerr << "opcode " << unsigned(opcode) << std::endl;
		...
		//std::cerr << "cycles " << unsigned(cycles) << std::endl;
